...
def main_worker(gpu, args):
    ...
    if args.dataset == 'cifar10':
        ...
        train_dataset = SemiSupervisedImbalanceChexpert(
            ...
            unlabel_imb_factor=args.imb_factor_unlabel
        )
        val_dataset = ChexpertDataset(ds_path / "valid.csv", "valid")
        train_sampler = None
        if args.train_rule == 'Resample':
            train_sampler = ImbalancedDatasetSampler(train_dataset)
        train_loader = torch.utils.data.DataLoader(
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, tf_writer):
    ...
    for i, (inputs, target) in enumerate(train_loader):
        ...
        output = model(inputs)
        label = torch.flatten(target.type(torch.cuda.LongTensor))
...
def validate(val_loader, model, criterion, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    with torch.no_grad():
        ...
        for i, (inputs, target) in enumerate(val_loader):
            ...
            output = model(inputs)
            label = torch.max(target.type(torch.cuda.LongTensor), dim=1)[1]
            loss = criterion(output, label)
