...
def log(log_type, val, epoch):
    print(f"{log_type}: {val} at epoch {epoch}")
...
def main():
    ...
    if opt.dataset == 'miniImageNet':
        ...
        if opt.distill in ['contrast']:
            train_set = Chexpert(args=opt, partition=train_partition, is_sample=True, k=opt.nce_k)
        else:
            train_set = Chexpert(args=opt, partition=train_partition)
        n_data = len(train_set)
        train_loader = DataLoader(train_set,
                                  batch_size=opt.batch_size, shuffle=True, drop_last=True,
                                  num_workers=opt.num_workers)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        log('train_acc', train_acc, epoch)
        log('train_loss', train_loss, epoch)

        test_acc, test_acc_top5, test_loss = validate(val_loader, model_s, criterion_cls, opt)

        log('test_acc', test_acc, epoch)
        log('test_acc_top5', test_acc_top5, epoch)
